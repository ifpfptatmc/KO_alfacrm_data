...
def fetch_customers_with_status_and_source():
    ...
    if response.status_code == 200:
        ...
        with open('customers_statuses_sources.csv', 'w', newline='') as csvfile:
            fieldnames = ['customer_id', 'status_id', 'source_id', 'custom_first_sum']
            ...
            writer.writerow({'customer_id': 'Last updated', 'status_id': '', 'source_id': '', 'custom_first_sum': datetime.now().strftime('%Y-%m-%d %H:%M:%S')})
